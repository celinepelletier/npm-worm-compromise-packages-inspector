// Usage: node parse-lock.js <repo/path> <file-path>
const fs = require('fs');
...
rawInput.trim().split('\n').forEach(line => {
  ...
  if (parts.length === 3) {
    const [, packageName, versionString] = parts;
    const versions = versionString.split(',').map(v => v.trim());
    filters[packageName] = versions;
  }
});
...
function extractDependencies(deps, collected = {}) {
  for (const [name, info] of Object.entries(deps || {})) {
    collected[name] = info.version;
    if (info.dependencies) {
      extractDependencies(info.dependencies, collected);
    }
  }
  return collected;
}
